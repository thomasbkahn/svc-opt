...
----
jinja embedding:
-use visualization methods with savetag='return' and collect output as dict
-make jinja template a la embed_multiple.py to insert head resources and plots where needed

later:
-ipython notebook demo
-polynomial regression to find "true" max (don't need to average results, just collect and fit with all, this will capture variation effects too)
-detail plots (single array in detail, with 1d plots highlighted upon hover) {later for second part, will need custom js}
-doc strings
